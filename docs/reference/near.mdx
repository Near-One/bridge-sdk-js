...
Build a transaction to finalize a UTXO deposit on NEAR. Calls `verify_deposit_v2` on the connector contract. The contract selects the safe vs. standard deposit path from `depositMsg.safe_deposit`; when a safe deposit is provided the builder attaches the NEAR required to pre-fund the user's token storage.
...
<ResponseField name="params" type="UtxoDepositFinalizationParams" required>
  <Expandable title="UtxoDepositFinalizationParams">
    ...
    <ResponseField name="depositMsg" type="UtxoDepositMsg" required>
      <Expandable title="UtxoDepositMsg">
        ...
        <ResponseField name="safe_deposit" type="UtxoSafeDeposit">
          Safe deposit message. When provided, `verify_deposit_v2` routes through the safe-deposit path (reverts on failure) and the builder attaches NEAR for the user's token storage.
        </ResponseField>

        <ResponseField name="refund_address" type="string">
    ...
    <ResponseField name="coinbaseTxId" type="string" required>
      Coinbase transaction ID for the block (first transaction).
    </ResponseField>

    <ResponseField name="coinbaseMerkleProof" type="string[]" required>
...
<ResponseField name="NearUnsignedTransaction" type="object">
  An unsigned verify_deposit_v2 transaction.
</ResponseField>
...
Build a transaction to verify a UTXO withdrawal on NEAR. Calls `verify_withdraw_v2` on the connector. Use `BtcBuilder.getWithdrawProof()` to obtain the proof fields.
...
<ResponseField name="NearUnsignedTransaction" type="object">
  An unsigned verify_withdraw_v2 transaction.
</ResponseField>
...
### toNearApiJsActions (near-api-js)

Convert `NearUnsignedTransaction` actions to plain objects compatible with near-api-js.

These plain objects serialize identically to `@near-js/transactions` Action class instances via Borsh, so they work directly with `Account.signAndSendTransaction()`.

#### Signature

```typescript
function toNearApiJsActions(unsigned: NearUnsignedTransaction): NearApiJsAction[]
```
...
<ResponseField name="NearApiJsAction[]" type="NearApiJsAction[]">
  Array of plain action objects. Cast to `Action[]` for TypeScript compatibility with near-api-js.
</ResponseField>
