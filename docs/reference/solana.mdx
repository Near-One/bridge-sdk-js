---
title: "@omni-bridge/solana"
description: Instruction builder for SVM chains (Solana and Fogo)
---
...
This package exposes two factories — `createSolanaBuilder` and `createFogoBuilder` — that share the same `SolanaBuilder` interface. Fogo is SVM-compatible and runs the same Anchor program as Solana; only the Wormhole core program and default RPC endpoint differ.
...
Factory function to create a Fogo instruction builder. Returns the same `SolanaBuilder` interface as `createSolanaBuilder` — Fogo runs the same Anchor program as Solana.

### Signature

```typescript
function createFogoBuilder(config: FogoBuilderConfig): SolanaBuilder
```

### Parameters

<ResponseField name="config" type="FogoBuilderConfig" required>
  <Expandable title="FogoBuilderConfig">
    <ResponseField name="network" type="'mainnet' | 'testnet'" required>
      The network to connect to. Fogo testnet is not yet deployed — passing `"testnet"` throws a `ValidationError` with code `UNSUPPORTED_CHAIN`.
    </ResponseField>

    <ResponseField name="connection" type="Connection">
      Optional Solana-web3.js `Connection`. If not provided, uses the Fogo public RPC:
...
<ResponseField name="SolanaBuilder" type="object">
  Same interface as `createSolanaBuilder` — all `buildTransfer`, `buildFinalization`, `buildLogMetadata`, `buildDeployToken`, and PDA derivation methods work identically. Source/destination chain is `ChainKind.Fogo` and addresses use the `fogo:` prefix.
</ResponseField>
...
// Default public RPC
const fogo = createFogoBuilder({ network: "mainnet" })

// Custom connection
const connection = new Connection("https://my-fogo-rpc.example.com")
const fogoCustom = createFogoBuilder({ network: "mainnet", connection })
```
...
Configuration for creating a Fogo builder. Same shape as `SolanaBuilderConfig`.

```typescript
interface FogoBuilderConfig {
  network: Network
