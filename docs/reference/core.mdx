...
description: Validation, types, configuration, and API client
---
...
```typescript
api.getTransferStatus(options: TransferLookupParams): Promise<TransferStatus[]>
```

### Parameters

<ResponseField name="options" type="TransferLookupParams" required>
  Lookup options. Provide `transactionHash`, OR `originChain` + `originNonce`,
  OR `utxoChain` + `utxoTxHash` + `utxoVout`.

  <Expandable title="options">
    <ResponseField name="transactionHash" type="string">
      Any transaction hash involved in the transfer.
    </ResponseField>
    ...
    <ResponseField name="utxoChain" type='"Btc" | "Zcash"'>
      UTXO chain of the reference.
    </ResponseField>

    <ResponseField name="utxoTxHash" type="string">
      UTXO transaction hash (deposit or payout).
    </ResponseField>

    <ResponseField name="utxoVout" type="number">
      UTXO output index.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="TransferStatus[]" type="TransferStatus[]">
  Array of status values, one per matching transfer. A nonce or UTXO-ref
  lookup matches a single transfer, so the array holds one element; a
  transaction-hash lookup can match several transfers if the transaction
  ...
  - `"Settled"` - Terminal state (fee claimed, or UTXO payout verified on NEAR)

  The set is open for extension — new statuses may be added server-side and
...
```typescript
api.getTransfer(options: TransferLookupParams): Promise<Transfer[]>
```
...
<ResponseField name="Transfer[]" type="Transfer[]">
  ...
  <Expandable title="Transfer properties">
    <ResponseField name="transfer_id" type="OmniTransferId | null">
      Canonical transfer id — a discriminated union on `type`: either
      `{ type: "nonce", chain, nonce }` or `{ type: "utxo", chain, tx_hash, vout }`.
      Null only on a partially-aggregated transfer whose identifying event
    ...
    <ResponseField name="native_fee" type="string | null">
      Relayer fee in the source chain's native token.
    </ResponseField>

    <ResponseField name="msg" type="string | null">
      Attached message, if any.
    </ResponseField>

    <ResponseField name="destination_nonce" type="number | null">
      Nonce assigned on the destination chain.
    </ResponseField>
    ...
    <ResponseField name="signed" type="TransactionRef[]">
      MPC signings of the transfer message. May hold several entries when the
      transfer was re-signed — the latest is `signed.at(-1)`.
    </ResponseField>

    <ResponseField name="fast_finalised_on_near" type="TransactionRef | null">
      Transaction when fast finalized on NEAR.
    </ResponseField>

    <ResponseField name="finalised_on_near" type="TransactionRef | null">
      Transaction when finalized on NEAR.
    </ResponseField>

    <ResponseField name="fast_finalised" type="TransactionRef | null">
      Transaction when fast finalized on destination.
    </ResponseField>

    <ResponseField name="finalised" type="TransactionRef | null">
      Transaction when fully finalized on the destination chain.
    </ResponseField>

    <ResponseField name="claimed" type="TransactionRef | null">
      Transaction when the relayer claimed their fee.
    </ResponseField>

    <ResponseField name="verified" type="TransactionRef | null">
      NEAR-side settlement of a transfer to a UTXO chain (`verify_withdraw`).
    </ResponseField>

    <ResponseField name="fee_updates" type="TransactionRef[]">
      Fee update transactions.
    </ResponseField>

    <ResponseField name="utxo_signs" type="UtxoSign[]">
      UTXO signing transactions (`TransactionRef` plus `destination_chain`,
      `relayer`, and `pending_sign_id`). Multiple entries are expected when a
      multi-UTXO withdrawal needs several signatures, and on re-sign (RBF).
    </ResponseField>

    <ResponseField name="utxo_winning_tx_hash" type="string | null">
      The UTXO transaction that won the payout race, if any.
    </ResponseField>

    <ResponseField name="utxo_meta" type="UtxoMeta | null">
      UTXO-specific metadata: `chain`, `pending_sign_id`, `relayer_fee`,
      `protocol_fee`, `relayer_account_id`.
    </ResponseField>

    <ResponseField name="related_txs" type="RelatedTx[]">
      Steps of the transfer that have no field of their own. The fields above
      hold one transaction per step, and a transfer through Hyperliquid has
      more. Each entry is a `TransactionRef` plus a `kind` saying which step it
      is. Empty for every other transfer.

      - `hyper_core_init` / `hyper_core_fin` — the HyperCore side of
        `initialised` / `finalised`. HyperCore has no blocks, so `details` is
        just `{ type: "hyper_core" }`.
      - `hyper_evm_init` — HyperEVM starts a transfer with two transactions.
        `initialised` holds the first one; this is the second.

      New kinds may be added without an SDK release, and pass through as plain
      strings.
    </ResponseField>

    <ResponseField name="tx_ids" type="string[]">
      Every transaction hash involved in the transfer.
    </ResponseField>
  </Expandable>
</ResponseField>
...
<ResponseField name="params" type="object" required>
  ...
  <Expandable title="params">
    ...
    <ResponseField name="limit" type="number">
      Maximum results to return. Defaults to `10`, capped at `50` by the API.
    </ResponseField>
  </Expandable>
</ResponseField>
...
```typescript
type ChainPrefix = "eth" | "near" | "sol" | "arb" | "base" | "bnb" | "btc" | "zcash" | "pol" | "hlevm" | "abs" | "strk" | "fogo" | "aptos"
```
...
```typescript
type Chain = "Eth" | "Near" | "Sol" | "Arb" | "Base" | "Bnb" | "Btc" | "Zcash" | "Pol" | "Abs" | "HlEvm" | "Strk" | "Fogo" | "Aptos"
```
...
`fogo` and `aptos` are optional: they are only present on networks where the
corresponding bridge contract is deployed.
...
## Hyperliquid Inbound Helpers

Helpers for bridging *into* Hyperliquid (HyperCore L1) from NEAR or any other chain. These run alongside the standard `Bridge.validateTransfer` flow; they only set the `recipient` and `message` fields correctly. For *outbound* HyperCore transfers (signed `sendToEvmWithData` actions), use [`@omni-bridge/hypercore`](/reference/hypercore).

### buildHyperliquidTransferParams

Build `TransferParams` for a transfer that lands on the recipient's HyperCore spot balance (instead of their HyperEVM EOA). Sets `recipient` to `hlevm:0x…` and `message` to the JSON envelope OmniBridge parses to route the destination-side mint through `HlBridgeToken`'s system-address pool.

#### Signature

```typescript
function buildHyperliquidTransferParams(params: {
  token: OmniAddress
  amount: bigint
  sender: OmniAddress
...
<ResponseField name="params" type="object" required>
  <Expandable title="params">
    <ResponseField name="token" type="OmniAddress" required>Source-chain token (e.g. `near:wrap.near`).</ResponseField>
    <ResponseField name="amount" type="bigint" required>Transfer amount in source-chain units.</ResponseField>
    <ResponseField name="sender" type="OmniAddress" required>Source-chain sender (e.g. `near:alice.near`).</ResponseField>
    <ResponseField name="hypercoreRecipient" type="string" required>The recipient's 20-byte EVM-style address on HyperEVM/HyperCore (`0x…`). HyperCore reuses the same address bytes as HyperEVM.</ResponseField>
    <ResponseField name="fee" type="bigint">Bridge fee in source-chain units. Defaults to `0n`. Quote via `bridge.api.getFee`.</ResponseField>
    <ResponseField name="nativeFee" type="bigint">Native (source-chain gas-token) fee. Defaults to `0n`.</ResponseField>
  </Expandable>
</ResponseField>

#### Returns

`TransferParams` ready to pass to `bridge.validateTransfer`. The returned `message` is `HYPERLIQUID_MESSAGE`.

#### Example

```typescript
import { createBridge, buildHyperliquidTransferParams } from "@omni-bridge/core"
...
const validated = await bridge.validateTransfer(
  buildHyperliquidTransferParams({
    ...
    amount: 1_000_000_000_000_000_000_000_000n, // 1 wNEAR (24 decimals)
    sender: "near:alice.near",
    hypercoreRecipient: "0xRecipient...",
    fee: 5_303_672_632_808_810_741_760n,
  }),
)
const tx = builder.buildTransfer(validated, "alice.near")
...
```typescript
const HYPERLIQUID_MESSAGE: string
```

The exact `TransferParams.message` value that routes the destination-side mint through `HlBridgeToken`'s system-address pool. Currently `'{"DestHexMsg":"636F7265"}'` — JSON envelope with `DestHexMsg` set to the hex of `"core"`. OmniBridge parses this envelope on HyperEVM and invokes the 3-arg `mint(addr, amt, bytes)` path; the resulting tokens are deposited into `_systemAddress` and Hyperliquid credits the recipient's HyperCore spot balance.

Exposed mostly for tests and indexer code that inspects `message`. End-user code should call `buildHyperliquidTransferParams` instead of setting `message` directly.
