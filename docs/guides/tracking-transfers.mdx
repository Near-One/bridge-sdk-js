...
| `Signed` | MPC signature obtained, ready for finalization |
| `FastFinalisedOnNear` | Fast finalized on NEAR (relayer provided liquidity) |
| `FinalisedOnNear` | Finalized on NEAR |
| `FastFinalised` | Fast finalized on destination |
| `Finalised` | Fully finalized on destination chain |
| `Claimed` | Fee claimed by relayer |

### Status Flow Diagrams

**Standard Flow (with relayer):**
```
...
**Fast Transfer Flow:**
```
Initialized → Signed → FastFinalisedOnNear → FastFinalised → FinalisedOnNear → Finalised → Claimed
```

**Manual Finalization Flow (no fees):**
```
Initialized → Signed → (you finalize) → Finalised
```

<Note>
  The `Signed` status indicates the MPC network has signed your transfer. At this point, you can finalize manually if you didn't include relayer fees.
</Note>
