...
Or by a UTXO reference (Bitcoin/Zcash deposits and payouts):
...
| `Settled` | Terminal state — relayer claimed their fee, or the UTXO payout was verified on NEAR |
...
async function waitForCompletion(txHash: string, timeoutMs = 300_000) {
  ...
  while (Date.now() - startTime < timeoutMs) {
    ...
    if (latest === "Finalised" || latest === "Settled") {
      return latest
    }
...
// `signed` is an array — the transfer may be re-signed
const signedTx = transfer?.signed.at(-1)?.transaction_hash
```
