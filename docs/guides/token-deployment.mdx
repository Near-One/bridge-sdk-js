...
## Deploying NEAR Tokens to Solana

When bridging a NEP-141 token from NEAR to Solana:

<Tabs>
  <Tab title="Step 1: Log Metadata (NEAR)">
    Call `log_metadata` on the NEAR bridge contract (same as EVM):

    ```typescript
    import { createNearBuilder, toNearKitTransaction } from "@omni-bridge/near"
    import type { LogMetadataEvent } from "@omni-bridge/near"

    const nearBuilder = createNearBuilder({ network: "mainnet" })
    const signerId = "deployer.near"

    // Build and send log_metadata transaction
    const tx = nearBuilder.buildLogMetadata("token.near", signerId)
    const result = await toNearKitTransaction(near, tx).send({ waitUntil: "FINAL" })
  ...
  <Tab title="Step 2: Deploy Token (Solana)">
    Use the signed metadata to deploy a wrapped SPL token on Solana:
    ...
    // Convert signature for Solana (no recovery ID adjustment needed)
    const signature = MPCSignature.fromRaw(metadataEvent.signature)

    // Derive the expected mint address (deterministic PDA)
    ...
    <Info>
      Solana wrapped tokens use deterministic PDA addresses. Use `deriveWrappedMint()` to get the address before deployment.
    </Info>
  </Tab>

  <Tab title="Handling Already Deployed">
    If the token is already deployed, the transaction will fail. Handle this gracefully:

    ```typescript
    try {
      const instructions = await solBuilder.buildDeployToken(signature, metadata, payer.publicKey)
      // ... send transaction
    } catch (error) {
      if (error.message?.match(/already deployed|already in use|AccountNotSystemOwned/i)) {
        // Token already exists - use deriveWrappedMint to get the address
        const mint = solBuilder.deriveWrappedMint(metadata.token)
        console.log("Token already deployed at:", mint.toBase58())
      } else {
        throw error
      }
    }
...
<Note>
  Unlike EVM deployment, Solana doesn't require a separate `bindToken` step. The token mapping is established during deployment.
</Note>
