  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (latestStatus === "Finalised" || latestStatus === "Claimed") {
      ...
      if (transfer?.finalised?.EVMLog) {
        console.log("  ETH TX:", transfer.finalised.EVMLog.transaction_hash)
      }
      return transfer
    }
...
| `toNearApiJsActions()` | @near-js/* | Converts to Action[] for signAndSendTransaction |
| `sendWithNearApiJs()` | @near-js/* | One-liner to convert and send |
...
// @near-js/*: get actions for manual handling
