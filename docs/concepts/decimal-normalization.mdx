...
Different blockchains use different decimal precisions for the same token. The Omni Bridge handles this automatically, but understanding it is critical for avoiding failed transfers.
...
When scaling down from high precision to low precision, small amounts may become zero:
