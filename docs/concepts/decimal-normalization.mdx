...
Different blockchains use different <Tooltip tip="The number of decimal places used to represent fractional token amounts. For example, 6 decimals means 1 token = 1,000,000 base units.">decimal precisions</Tooltip> for the same token. The Omni Bridge handles this automatically, but understanding it is critical for avoiding failed transfers.
