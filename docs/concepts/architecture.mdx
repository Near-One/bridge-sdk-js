...
The Omni Bridge SDK uses a **<Tooltip tip="A design pattern where the SDK handles all protocol logic and encoding, but returns unsigned transactions for you to sign with your own wallet or key management system.">transaction builder pattern</Tooltip>**. This design separates the bridge protocol logic from transaction signing, giving you complete control over your key management.
...
<Tree>
  <Tree.Folder name="@omni-bridge" defaultOpen>
    ...
    <Tree.File name="near   → NEAR Protocol + shims" />
    <Tree.File name="solana → Solana (Anchor-based)" />
    <Tree.File name="btc    → Bitcoin, Zcash (UTXO)" />
