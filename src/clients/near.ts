...
export class NearBridgeClient {
	...
	constructor(
		...
		this.bitcoinService = new BitcoinService(
			addresses.btc.apiUrl,
			addresses.btc.network,
		);
	}
	...
	get networkId(): string {
		return (
			(this.wallet as { connection?: { networkId?: string } }).connection
				?.networkId || "unknown"
		);
	}
	...
	async finalizeBitcoinDeposit(
		...
		const rawBitcoinTx =
			await this.bitcoinService.getTransactionBytes(btcTxHash);
	...
	async initBitcoinWithdrawal(
		...
		const msg: InitBtcTransferMsg = {
			Withdraw: {
				...
				output: outputs.map((o: Output) => ({
					...
					script_pubkey: Array.from(
						this.bitcoinService.addressToScriptPubkey(o.address),
		...
		const totalAmount =
			amount + (fee ?? 0n) + BigInt(bitcoinConfig.withdraw_bridge_fee.fee_min);
		...
		const btcPendingTxData = JSON.parse(
			btcPendingTxLog.split("EVENT_JSON:")[1],
		);
	...
	async waitForBitcoinTransactionSigning(
		...
		throw new Error(
			"Bitcoin: Unexpected end of waitForBitcoinTransactionSigning",
		);
	}
	...
	async executeBitcoinWithdrawal(
		...
		const btcPendingId = await this.initBitcoinWithdrawal(
			targetBtcAddress,
			amount,
		);
