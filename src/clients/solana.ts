...
export class SolanaBridgeClient {
  ...
  private static getConstant(name: string) {
    const value = (
      BRIDGE_TOKEN_FACTORY_IDL as BridgeTokenFactory
    ).constants.find((c) => c.name === name)?.value;
  ...
  async finalizeTransfer(
    ...
    const nonceGroup =
      payload.destination_nonce / BigInt(USED_NONCES_PER_ACCOUNT);
  ...
  private async isBridgedToken(token: PublicKey): Promise<boolean> {
    ...
    return (
      ...
      data.parsed.info.mintAuthority.toString() ===
      this.authority()[0].toString()
    );
  }

  private async getTokenProgramForMint(mint: PublicKey): Promise<PublicKey> {
    const accountInfo =
      await this.program.provider.connection.getAccountInfo(mint);
    if (!accountInfo) {
      throw new Error("Failed to find mint account");
    }
