  ...
  vout: [
    {
      ...
      scriptpubkey_asm:
        "OP_DUP OP_HASH160 OP_PUSHBYTES_20 abcdef1234567890abcdef1234567890abcdef OP_EQUALVERIFY OP_CHECKSIG",
...
describe("NearBridgeClient Bitcoin Methods", () => {
  ...
  beforeEach(() => {
    ...
    client = new NearBridgeClient(mockNear, REAL_TEST_DATA.bridgeContract, {
      defaultSignerId: REAL_TEST_DATA.testAccount,
    })
    ...
    client.bitcoinService.buildWithdrawalPlan = vi.fn().mockReturnValue({
      inputs: ["abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890:0"],
      outputs: [
        {
          value: 50000,
  ...
  describe("getUtxoBridgeConfig (btc)", () => {
    it("should fetch Bitcoin bridge configuration", async () => {
      // Mock the NEAR contract view call
      mockNear.view = vi.fn().mockResolvedValue(mockBtcConnectorConfig)
    ...
    it("should handle config fetch errors", async () => {
      mockNear.view = vi.fn().mockRejectedValue(new Error("Contract not found"))
  ...
  describe("getUtxoDepositAddress (btc)", () => {
    ...
    it("should get Bitcoin deposit address for recipient", async () => {
      mockNear.view = vi.fn().mockResolvedValue(mockDepositResponse.deposit_address)
    ...
    it("should handle invalid recipient ID", async () => {
      mockNear.view = vi.fn().mockRejectedValue(new Error("Invalid account ID"))
    ...
    it("should work with different recipient types", async () => {
      ...
      for (const recipient of recipients) {
        mockNear.view = vi.fn().mockResolvedValue(`tb1q${recipient.slice(0, 10)}`)
  ...
  describe("UTXO helpers", () => {
    it("should surface unified UTXO deposit address API", async () => {
      mockNear.view = vi.fn().mockResolvedValue("tb1qunified")

      const result = await client.getUtxoDepositAddress(ChainKind.Btc, REAL_TEST_DATA.testAccount)

      expect(result.depositAddress).toBe("tb1qunified")
      expect(result.depositArgs).toEqual({
        deposit_msg: { recipient_id: REAL_TEST_DATA.testAccount },
      })
    })
  ...
  describe("finalizeBitcoinDeposit", () => {
    ...
    it("should finalize Bitcoin deposit with real transaction data", async () => {
      ...
      mockTransactionBuilder.send.mockResolvedValueOnce(mockTransactionResult)
      ...
      // Verify the transaction builder was called correctly
      expect(mockNear.transaction).toHaveBeenCalledWith(REAL_TEST_DATA.testAccount)
    ...
    it("should handle different vout values", async () => {
      ...
      mockTransactionBuilder.send.mockResolvedValue({
        transaction: { hash: "test_hash" },
      })

      for (const vout of [0, 1]) {
        // Only test valid vout indices that exist in mockBitcoinTx
  ...
  describe("initUtxoWithdrawal (btc)", () => {
    it("should initialize Bitcoin withdrawal", async () => {
      ...
      mockNear.view = vi.fn().mockImplementation((_contractId: string, methodName: string) => {
      ...
      mockTransactionBuilder.send.mockResolvedValueOnce(mockNearTxResult)
      ...
      expect(mockNear.transaction).toHaveBeenCalledWith(REAL_TEST_DATA.testAccount)
      expect(mockTransactionBuilder.functionCall).toHaveBeenCalled()
    })
    ...
    it("should include Bitcoin label when change address is missing", async () => {
      const configSpy = vi.spyOn(client, "getUtxoBridgeConfig").mockResolvedValue({
        ...mockBtcConnectorConfig,
        change_address: "",
      })
      const utxoSpy = vi.spyOn(client, "getUtxoAvailableOutputs").mockResolvedValue([
        {
          path: "m/44'/1'/0'/0/0",
          tx_bytes: new Uint8Array([2, 0, 0, 0, 1]),
          vout: 0,
    ...
    it("should forward bridge config overrides to the planner", async () => {
      ...
      mockTransactionBuilder.send.mockResolvedValueOnce({
  ...
  describe("waitForUtxoTransactionSigning (btc)", () => {
    it("should wait for and find Bitcoin transaction signing", async () => {
      ...
      server.use(
        http.get(
          "https://testnet.api.bridge.nearone.org/api/v3/transfers/transfer",
          ({ request }) => {
      ...
      const nearTxHash = await client.waitForUtxoTransactionSigning(
        ChainKind.Btc,
  ...
  describe("finalizeUtxoWithdrawal (btc)", () => {
    it("should finalize Bitcoin withdrawal and broadcast transaction", async () => {
      ...
      mockNear.getTransactionStatus = vi.fn().mockResolvedValue(mockNearTxStatus)

      const bitcoinTxHash = await client.finalizeUtxoWithdrawal(
        ChainKind.Btc,
    ...
    it("should throw error when signed transaction not found in logs", async () => {
      ...
      await expect(
        client.finalizeUtxoWithdrawal(ChainKind.Btc, "invalid_tx_hash", REAL_TEST_DATA.testAccount),
      ).rejects.toThrow("Bitcoin: Signed transaction not found in NEAR logs")
    })
  })
  ...
  describe("executeUtxoWithdrawal (btc)", () => {
    it("should execute complete Bitcoin withdrawal flow", async () => {
      ...
      mockTransactionBuilder.send.mockResolvedValueOnce(mockInitResult)
      mockNear.getTransactionStatus = vi.fn().mockResolvedValue(mockFinalizeStatus)
    ...
    it("should handle errors in any step of the flow", async () => {
      ...
      mockTransactionBuilder.send.mockRejectedValueOnce(new Error("Init failed"))
        ...
        receipts_outcome: [
          {
            outcome: {
              logs: [
                'EVENT_JSON:{"standard":"nep297","version":"1.0.0","event":"generate_btc_pending_info","data":[{"btc_pending_id":"large_amount_test"}]}',
              ],
            },
          },
        ],
        ...
        receipts_outcome: [
          {
            outcome: {
              logs: [
                'EVENT_JSON:{"standard":"nep297","version":"1.0.0","event":"generate_btc_pending_info","data":[{"btc_pending_id":"min_amount_test"}]}',
              ],
            },
          },
        ],
  ...
  describe("Network Configuration", () => {
    it("should work with testnet configuration", () => {
      // Verify client is set up with correct bridge contract
      expect(client.bridgeContractId).toBe(REAL_TEST_DATA.bridgeContract)
    })
