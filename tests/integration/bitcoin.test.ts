  ...
  vin: [
    {
      ...
      vout: 0,
      scriptsig:
        "473044022012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678",
  ...
  vout: [
    {
      ...
      scriptpubkey_asm:
        "OP_DUP OP_HASH160 OP_PUSHBYTES_20 sender_address_hash160... OP_EQUALVERIFY OP_CHECKSIG",
...
const server = setupServer(
  ...
  http.get(
    `https://blockstream.info/testnet/api/tx/${REAL_WITHDRAWAL_DATA.realDepositTx}/merkle-proof`,
    () => {
  ...
  http.get(
    `https://blockstream.info/testnet/api/tx/${REAL_WITHDRAWAL_DATA.realDepositTx}/hex`,
    () => {
      // Simplified hex representation
      return new HttpResponse(
        "0200000001f1e2d3c4b5a6978869504132210fedcba9876543210fedcba987654321abcdef00000000",
        {
...
describe("Bitcoin Integration Tests", () => {
  ...
  describe("End-to-End Deposit Flow", () => {
    ...
    it("should validate deposit amounts against bridge configuration", async () => {
      ...
      expect(depositOutputValue).toBeGreaterThan(Number.parseInt(config.min_deposit_amount))
    })
  })
  ...
  describe("End-to-End Withdrawal Flow", () => {
    ...
    it("should use executeBitcoinWithdrawal for complete automation", async () => {
      ...
      server.use(
        http.get(
          `https://api-testnet.nearblocks.io/v1/account/${REAL_WITHDRAWAL_DATA.relayerAccount}/receipts`,
          () => {
            return HttpResponse.json({
              txns: [
                {
    ...
    it("should validate withdrawal amounts against bridge configuration", async () => {
      ...
      await expect(
        client.initBitcoinWithdrawal(REAL_WITHDRAWAL_DATA.realWithdrawAddress, belowMinimum),
  ...
  describe("Error Scenarios and Recovery", () => {
    it("should handle Bitcoin network failures gracefully", async () => {
      ...
      server.use(
        http.get(
          `https://blockstream.info/testnet/api/tx/${REAL_WITHDRAWAL_DATA.realDepositTx}`,
          () => {
    ...
    it("should handle NEAR contract failures", async () => {
      ...
      mockWallet.provider.callFunction = vi
        .fn()
        .mockRejectedValue(new Error("Contract method not found"))
  ...
  describe("Cross-Chain Integration", () => {
    it("should work within broader bridge context", async () => {
      ...
      expect(mockWallet.provider.callFunction).toHaveBeenNthCalledWith(
        1,
        "brg-dev.testnet",
        "get_config",
        {},
  ...
  describe("Real Data Validation", () => {
    ...
    it("should use realistic amounts from bridge configuration", () => {
      expect(Number.parseInt(REAL_WITHDRAWAL_DATA.minDepositAmount)).toBeGreaterThan(1000) // > 1000 sats
      expect(Number.parseInt(REAL_WITHDRAWAL_DATA.minWithdrawAmount)).toBeGreaterThan(
        Number.parseInt(REAL_WITHDRAWAL_DATA.minDepositAmount),
      )
    })
  ...
  describe("Performance and Scalability", () => {
    it("should handle multiple concurrent operations", async () => {
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => client.getBitcoinBridgeConfig())
        ...
        receipts_outcome: [
          {
            outcome: {
              logs: [
                'EVENT_JSON:{"standard":"nep297","version":"1.0.0","event":"generate_btc_pending_info","data":[{"btc_pending_id":"large_amount_pending"}]}',
              ],
            },
          },
        ],
